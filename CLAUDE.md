...
- `traefik` — **repo-side redesign done (commit `9029c3f`), host-side steps not yet done.** `TRAEFIK_DASHBOARD_CREDENTIALS` was baked into a `labels:` entry via Compose `${VAR}` substitution, which needs a `stack.env` committed to the repo for Repository-mode GitOps — not viable in a public repo. Fixed by moving dashboard basic-auth off the label and into `config.yml`'s file-provider middleware (`traefik-auth: basicAuth.usersFile: /run/secrets/traefik_dashboard_htpasswd`), backed by a new Docker secret, same pattern as `cf_api_token`. `CF_DNS_API_TOKEN_FILE` was already fine as an absolute-path secret. Along the way, also fixed a `.gitignore` bug where `**/secrets/` excluded the whole directory and silently defeated the `!**/secrets/*-example` negation underneath it — NextCloud's example secret file had never actually been committed as a result; now fixed (`**/secrets/*`) and both traefik's and NextCloud's `*-example` files are tracked.

  **Still to do for traefik, in order:**
  1. On nelson-nuc, create `/home/nelson/containers/traefik/dashboard_htpasswd.txt` containing the existing `user:bcrypt-hash` line (same value currently backing `TRAEFIK_DASHBOARD_CREDENTIALS`, wherever that's set today — likely Portainer's env var UI, since this stack isn't GitOps-connected yet).
  2. Copy the repo's updated `stacks/nelson-nuc/traefik/config.yml` to `/home/nelson/containers/traefik/data/config.yml` on the host. **Important pre-existing gap found while doing this:** the running traefik container's `config.yml`, `traefik.yml`, and `acme.json` are all bind-mounted from `/home/nelson/containers/traefik/data/` on the host, not from this repo — edits to the repo copies are cosmetic until manually synced there. Worth a proper fix later (maybe as part of connecting traefik to GitOps), but out of scope for this pass.
  3. Redeploy the traefik stack (still via Portainer's local editor for now — not GitOps-connected yet) and confirm the dashboard still prompts for basic auth and accepts the same credentials as before.
  4. Only after that's confirmed working: connect `traefik` to Portainer GitOps (Repository mode, same flow as `mealie`/`cloudflared`).

- `reactive-resume` — not started. Uses `${VAR}` substitution across 4 services, including values composed into other values (e.g. `DATABASE_URL` embedding `${POSTGRES_PASSWORD}`) and the same secret reused under different variable names per service (`MINIO_ROOT_USER`/`STORAGE_ACCESS_KEY`, `CHROME_TOKEN`/`TOKEN`). Needs consolidation into a single literal `.env` per service (env_file can't do string interpolation, so composed/reused values must be written out explicitly under each key). Note: an upstream commit ("rective resume fix", 2026-06-28) changed this stack's `docker-compose.yml` and trimmed its `FIXME.md` after the `${VAR}` analysis above was done — re-read current state before converting.
